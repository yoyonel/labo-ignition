...
RUN ARCH=$([ "${TARGETARCH}" = "arm64" ] && echo "aarch64" || echo "x86_64") \
    ...
    && JUMP_VERSION=$(curl -s https://api.github.com/repos/gsamokovarov/jump/releases/latest | jq -r '.tag_name | ltrimstr("v")') \
    ...
    && PROCS_URL=$(curl -s https://api.github.com/repos/dalance/procs/releases/latest | jq -r --arg arch "${ARCH}" '[.assets[] | select(.name | test($arch)) | select(.name | test("linux")) | select(.name | endswith(".zip")) | .browser_download_url] | first') \
    && curl -L "${PROCS_URL}" -o /tmp/procs.zip \
    && unzip /tmp/procs.zip -d /tmp/procs-pkg && mv /tmp/procs-pkg/procs /usr/local/bin/ && rm -rf /tmp/procs* \
    && LSD_URL=$(curl -s https://api.github.com/repos/lsd-rs/lsd/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-gnu.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
...
RUN ARCH=$([ "${TARGETARCH}" = "arm64" ] && echo "aarch64" || echo "x86_64") \
    ...
    && EZA_URL=$(curl -s https://api.github.com/repos/eza-community/eza/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-gnu.tar.gz" '.assets[] | select(.name | endswith($arch)) | select(.name | contains("man") | not) | .browser_download_url') \
    && curl -L "${EZA_URL}" -o /tmp/eza.tar.gz \
    && tar -xzf /tmp/eza.tar.gz -C /usr/local/bin/ && chmod +x /usr/local/bin/eza && rm -rf /tmp/eza* \
    && ZOXIDE_URL=$(curl -s https://api.github.com/repos/ajeetdsouza/zoxide/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-musl.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
    && curl -L "${ZOXIDE_URL}" -o /tmp/zoxide.tar.gz \
    && tar -xzf /tmp/zoxide.tar.gz -C /tmp/ && mv /tmp/zoxide /usr/local/bin/ && rm -rf /tmp/zoxide* \
    && DELTA_URL=$(curl -s https://api.github.com/repos/dandavison/delta/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-musl.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
    && curl -L "${DELTA_URL}" -o /tmp/delta.tar.gz \
    && tar -xzf /tmp/delta.tar.gz -C /tmp/ && mv /tmp/delta-*/delta /usr/local/bin/ && rm -rf /tmp/delta* \
    && DUST_URL=$(curl -s https://api.github.com/repos/bootandy/dust/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-musl.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
    && curl -L "${DUST_URL}" -o /tmp/dust.tar.gz \
    && tar -xzf /tmp/dust.tar.gz -C /tmp/ && mv /tmp/dust-*/dust /usr/local/bin/ && rm -rf /tmp/dust* \
    && HF_URL=$(curl -s https://api.github.com/repos/sharkdp/hyperfine/releases/latest | jq -r --arg arch "${ARCH}-unknown-linux-musl.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
    ...
    && GLOW_URL=$(curl -s https://api.github.com/repos/charmbracelet/glow/releases/latest | jq -r --arg arch "Linux_${GLOW_ARCH}.tar.gz" '.assets[] | select(.name | endswith($arch)) | .browser_download_url') \
